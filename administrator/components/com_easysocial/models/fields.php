	...
	public function addFieldsDefault($workflowId, $fieldGroup, $data, $datakey = array())
	{
		...
		foreach ($data as $fieldId => $values) {
			...
			foreach ($keys as $key) {
				...
				$query = "insert into `#__social_fields_data` (`field_id`, `uid`, `type`, `data`, `raw`, `datakey`) ";

				if ($fieldGroup == SOCIAL_FIELDS_GROUP_USER) {
					// users

					$query .= " select distinct " . $db->Quote($fieldId) . ", `user_id`, " . $db->Quote($fieldGroup) . ", " . $db->Quote($val) . ", " . $db->Quote($raw) . ", " . $db->Quote($key);
					...
					// clusters
					$query .= " select distinct " . $db->Quote($fieldId) . ", a.`id`, " . $db->Quote($fieldGroup) . ", " . $db->Quote($val) . ", " . $db->Quote($raw)  . ", " . $db->Quote($key);
