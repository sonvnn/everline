...
class EasySocialModelUsers extends EasySocialModel
{
	...
	public function getUsers($options = array())
	{
		...
		if (!$blockedUser && ES::config()->get('users.blocking.enabled') && $excludeBlocked && $my->id) {
		...
		if ($blockedUser && ES::config()->get('users.blocking.enabled') && $my->id) {
	 ...
	 * @access	public
	 */
	public function getUpcomingBirthdays($key, $userId, $limit = null, $page = null, $limitstart = null)
	{
		$db = ES::db();
		$config = ES::config();
		$my = ES::user();
		...
		$query[0] .= ", (366 + DAYOFYEAR(fd.`raw`) - DAYOFYEAR(" . $db->Quote($now) . ")) % 366 as `left_days`";
