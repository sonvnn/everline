...
class SocialTableComments extends SocialTable
{
	...
	public function getFormattedComment($viewer)
	{
		...
		if ($tags) {
			$formattedComment = $stringLib->processTags($tags, $formattedComment, true, 'rest');
		}
		...
		// Finalize the format
		$commentObject = $this->formatCommentObjects($viewer, $formattedComment, $tags);

		$commentContent->formatted = $formattedComment;
