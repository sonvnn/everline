...
class SocialUserAppPhotos extends SocialAppItem
{
	...
	{
		$allowed = array('stream.user.upload', 'stream.page.upload', 'stream.group.upload', 'stream.event.upload',
	...
	public function onNotificationLoad(SocialTableNotification &$item)
	{
		...
		// Comments
		if (($item->cmd == 'albums.comment.add' || $item->cmd == 'photos.comment.add' || $item->cmd == 'comments.item' || $item->cmd == 'comments.involved')) {
	...
	public function onPrepareRestStream(SocialStreamItem &$item, $includePrivacy = true, $viewer)
	{
		...
		// Get full photos from params if its story album
		// if ($album->isStory()) {
		// 	$limit = 0;
		// }
