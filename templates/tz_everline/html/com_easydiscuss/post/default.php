...
>
    <?php if (JComponentHelper::isInstalled('com_tz_envato_license')) {
        $app    =   JFactory::getApplication();
        $id     =   $app->input->get('id',0);
	    $user       =   JFactory::getUser();
	    $db     =   JFactory::getDbo();
	    $query  =   $db -> getQuery(true);
	    $query ->   select('e.*, p.name');
	    $query ->   from($db->quoteName('#__tz_envato_product_discuss', 'd'));
	    $query ->   join('INNER', $db->quoteName('#__tz_envato_product_detail', 'p') . ' ON ' . $db->quoteName('d.product_id') . ' = ' . $db->quoteName('p.id'));
	    $query ->   join('INNER', $db->quoteName('#__tz_envato_manager', 'e') . ' ON ' . $db->quoteName('e.itemid') . ' = ' . $db->quoteName('p.envato_id'));
	    $query ->   join('INNER', $db->quoteName('#__discuss_posts', 'dp') . ' ON ' . $db->quoteName('dp.category_id') . ' = ' . $db->quoteName('d.discuss_id'));
	    $query ->   where('dp.id='.$db -> quote($id));
	    $query ->   where('e.user_id=dp.user_id');
	    $query ->   where('e.supported_until>NOW()');
	    $db    ->   setQuery($query);
	    $license    =   $db -> loadObject();
	    if ($license) { ?>
            <div class="alert alert-success" role="alert">
                <p>This forum is private. The discussion will only be visible by the creator and the moderator of the topic.</p>
                <strong>License validated:</strong>
                <ul>
                    <li>Product Name: <?php echo $license->name; ?></li>
                    <li>Purchase Code: <?php echo $license->purchase_code; ?></li>
                    <li>License Type: <?php echo $license->license_type; ?></li>
                    <li>Purchase Date: <?php echo $license->purchase_date; ?></li>
                    <li>Supported Until: <?php echo $license->supported_until; ?></li>
                </ul>
            </div>
        <?php } ?>
    <?php } ?>
