...
class SppagebuilderAddonFeature extends SppagebuilderAddons {
	...
	public function render() {
		...
		$use_rellax = (isset($settings->use_rellax) && $settings->use_rellax) ? $settings->use_rellax : 0;
		$rellax_speed = (isset($settings->rellax_speed) && $settings->rellax_speed != '') ? $settings->rellax_speed : -2;
		$rellax_min = (isset($settings->rellax_min) && $settings->rellax_min != '') ? $settings->rellax_min : '';
		$rellax_max = (isset($settings->rellax_max) && $settings->rellax_max != '') ? $settings->rellax_max : '';
		$rellax_zindex = (isset($settings->rellax_zindex) && $settings->rellax_zindex != '') ? $settings->rellax_zindex : '';
		...
		//Rellax
		$rellax         =   $use_rellax ? 'rellax' : '';
		$rellax_data    =   '';
		if ($use_rellax) {
			$rellax_data .= $rellax_speed != '' ? ' data-rellax-speed="'.$rellax_speed.'"' : '';
			$rellax_data .= $rellax_min != '' ? ' data-rellax-min="'.$rellax_min.'"' : '';
			$rellax_data .= $rellax_max != '' ? ' data-rellax-max="'.$rellax_max.'"' : '';
			$rellax_data .= $rellax_zindex != '' ? ' data-rellax-zindex="'.$rellax_zindex.'"' : '';
		}

		//Output
		$output  = '<div class="sppb-addon sppb-addon-feature ' . $alignment . ' ' . $class . ' '. $rellax.'"'.$rellax_data.'>';
